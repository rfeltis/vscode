...
`SessionView` forwards chat input operations such as `prefillInput(...)` and `getInputTargetElement()` to its current concrete chat view. Overlay-style contributions should use this API to align with or fill the visible composer stack, including the new-session workspace picker, instead of querying chat DOM structure directly.
