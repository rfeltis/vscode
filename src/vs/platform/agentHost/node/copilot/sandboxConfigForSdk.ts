 ...
 *
 * The SDK ships this as an opaque object (its `SessionUpdateOptionsParams.sandboxConfig`
 * is typed as `{ [k: string]: unknown }`), but the runtime expects this concrete shape —
 * the same one the Copilot extension produces via its own `buildSandboxConfigForCLI`.
 */
